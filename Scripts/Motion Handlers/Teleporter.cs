...
public class Teleporter : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (disk.GetState(SteamVR_Input_Sources.RightHand) || disk.GetState(SteamVR_Input_Sources.LeftHand))
        {
            ...
            bool blocked = NavMesh.Raycast(player.transform.position, m_Pointer.transform.position, out navMeshHit,  NavMesh.AllAreas);
           ...
           // Debug.Log("Pointer: " + m_Pointer.transform.position);
            if(dist < maxDis){
                //     Debug.Log("LESS THAN 5");
                if( NavMesh.SamplePosition(hit.point, out navMeshHit, 1.0f, walkableArea)){
                    TryTeleport(navMeshHit);
                } 
    ...
    private void TryTeleport(NavMeshHit nav){
        ...
        Vector3 trans = nav.position - groundPos;
